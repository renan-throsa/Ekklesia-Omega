    ...
    public class OccasionValidation : AbstractValidator<OccasionDTO>
    {
        public OccasionValidation()
        {
            RuleSet(OperationType.Insert.ToString(), () =>
                        {
                            ...
                            .NotEmpty().WithMessage("Uma ocasião precisa ter ao menos um participante.")
                            .When(r => r.Type != OccasionType.CULT);

                            RuleFor(r => r.Host).NotNull().WithMessage("Uma ocasião precisa ter um responsável.");

                            RuleFor(r => r.Host.Name).NotEmpty().When(r => r.Host != null).WithMessage("Uma ocasião precisa ter um dirigente válido.");

                            RuleFor(r => r.Host.Id).NotEmpty().When(r => r.Host != null).WithMessage("Uma ocasião precisa ter um dirigente com Id válido.");
            ...
            RuleSet(OperationType.Update.ToString(), () =>
            {
                ...
                RuleFor(r => r.Attendees).NotEmpty().WithMessage("Uma ocasião precisa ter ao menos um participante.").When(r => r.Type != Enums.OccasionType.CULT);
